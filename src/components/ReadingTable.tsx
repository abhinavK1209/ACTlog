...
interface Props {
  ...
  onUpdate: (entry: ReadingEntry) => void;
...
export function ReadingTable({ entries, onUpdate, onDelete }: Props) {
    ...
    <>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={10} className="empty-state">No entries yet. Add one above.</td></tr>
            ) : filtered.map((e) => (
              <tr key={e.id} className={e.reviewed === 'no' ? 'unreviewed' : ''}>
                ...
                <td style={{ fontSize: '0.82rem' }}>{e.proofLocation}</td>
                <td style={{ maxWidth: 180, fontSize: '0.82rem' }}>{e.whyMissed}</td>
                <td style={{ maxWidth: 180, fontSize: '0.82rem' }}>{e.fixNextTime}</td>
...
function EditModal({ entry, onChange, onSave, onClose }: {
  entry: ReadingEntry;
  onChange: (e: ReadingEntry) => void;
  onSave: () => void;
  onClose: () => void;
}) {
  function set(key: keyof ReadingEntry, val: string) {
    onChange({ ...entry, [key]: val });
  }
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <h3>Edit Reading Entry</h3>
        <div className="form-grid">
          ...
          <div className="form-group"><label>Proof Location</label><input value={entry.proofLocation} onChange={(e) => set('proofLocation', e.target.value)} /></div>
          ...
          <div className="form-group"><label>Fix Next Time</label><textarea value={entry.fixNextTime} onChange={(e) => set('fixNextTime', e.target.value)} /></div>
          <div className="form-group">
